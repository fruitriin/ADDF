...
- **@メンション解決のパストラバーサル耐性**（Plan 0043 項目4）:
  `.claude/addf/addfTools/lint-template-sync.py` の `_repo_declaration_lines` と
  `.claude/addf/addfTools/verify-checksums.sh` の `detect_repo_kind()` の両側で、
  CLAUDE.repo.md 内の @メンション解決に `..` を含むパス・絶対パス・シンボリックリンク
  経由の脱出を silent に無視するガードを追加。ペア7 の同期契約で両側の整合を保つ
