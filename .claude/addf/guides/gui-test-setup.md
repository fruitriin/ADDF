...
```bash
bash .claude/addf/tools/verify-checksums.sh
```

### 3. Screen Recording 権限の確認

```bash
bash .claude/addf/tools/check-screen-recording.sh
```
...
```bash
# ウィンドウ情報を取得
.claude/addf/tools/window-info <プロセス名>

# スクリーンショットを撮影
.claude/addf/tools/capture-window <プロセス名> tmp/capture.png

# グリッドアノテーション
.claude/addf/tools/annotate-grid tmp/capture.png tmp/annotated.png

# 画像クリップ
.claude/addf/tools/clip-image tmp/annotated.png tmp/clip.png --rect 100,200,300,400
```
...
`addf-gui-test.md` スキルのプラットフォーム判定ロジックは実装済みのため、プラットフォーム固有ツールを `.claude/addf/tools/` に追加するだけで対応できます。
