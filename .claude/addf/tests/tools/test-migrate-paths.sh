...
check "案内が新位置のツールパスを示す" 0 "$code" "$out" ".claude/addf/tools/migrate-paths.py rewrite"
...
expect "ツール自身も新位置へ移動" test -f "$box/.claude/addf/tools/migrate-paths.py"
...
out="$(runpy "$box" ".claude/addf/tools/migrate-paths.py" rewrite)"; code=$?
check "新位置からの rewrite が exit 0" 0 "$code" "$out" "書き換えました"
check "完了案内が新位置の lint パスを示す" 0 "$code" "$out" ".claude/addf/tools/lint-residual-paths.py"
