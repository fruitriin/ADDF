...
depends_on:
  - .claude/addf/tools/lint-template-sync.py
  ...
  - .claude/addf/tools/speculate-guard.py
  - .claude/addf/tools/lint-toml.py
status: active
---
...
`.claude/addf/tools/` はダウンストリームに配布される。ADDF 本体固有ファイル（`ProgressTemplate.addf.md`・`AGENTS.md` 等）をハードコード参照すると、ダウンストリームで必ず ERROR になる。設計ルール:
...
- [スキル設計パターン（Anthropic 社内知見ベース）](skill-design-patterns.md) — スクリプトを `.claude/addf/tools/` に同梱する Progressive Disclosure 構成
