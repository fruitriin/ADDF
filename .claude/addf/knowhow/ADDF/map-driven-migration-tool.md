...
### rewrite の射程外 — 3類型（フェーズ2 実施で実測）

マップ駆動 rewrite は「フルパス文字列のリテラル出現」しか書き換えられない。本体移行の実施では
run-all 全19スイート中18が移行直後に失敗し、原因は全て以下の3類型だった（lint-residual-paths も
同じ理由で検出できない — フルパス文字列が存在しないため）:
...
`grep -rn "os.path.join.*'\.claude'"` と `grep -rn 'SCRIPT_DIR/\.\.'` 相当の断片走査を
プリフライトに足すと、事後デバッグではなく事前修正にできる。
