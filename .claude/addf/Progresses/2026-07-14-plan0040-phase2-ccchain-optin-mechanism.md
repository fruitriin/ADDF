...
### 現在のタスク: Plan 0040 フェーズ2（ccchain オプトイン配布機構）

#### サブタスクチェックリスト

- [x] `addf-Behavior.toml` に `[ccchain]`（既定 `enable = false`）セクションを新設
- [x] `.claude/addf/optional/ccchain/.ccchain.conf` にフェーズ1のチューニング済み設定を
      一般化して配置（プロジェクト固有ルール追加を促すコメント付き）
- [x] `sync-ccchain.py` 新設（check/apply。設計判断3点: settings.json のみ対象・
      .ccchain.conf は初回配置後上書きしない・既存フックを壊さず追加/削除のみ）
- [x] サンドボックスで配置→配線→バイナリ検知→撤去の全シナリオを手動検証
- [x] `test-sync-ccchain.sh` 新設（8シナリオ・23アサーション）。run_sync ヘルパーの
      `cd -` が `$?` を上書きするバグを発見・修正
- [x] `/addf-lint` セクション13・結果報告表に追加
- [x] `.gitignore` の ADDF マーカーブロックへ `/ccchain`・`.ccchain.local.conf` を移動（配布対象化）
- [x] `addf-init.md`・`addf-migrate.md` のコピーリスト・完了案内を更新
- [x] Plan 0040 の「要オーナー確認」（バイナリ不在時フェイルセーフ方針）をエージェント判断で解消
      （警告して素通し。他ツールの SKIP-first 方針との一貫性を根拠に）
- [x] knowhow 更新（フェーズ1/フェーズ2の配線先を意図的に分離している設計を明記。
      forked lint エージェントが「矛盾」として誤検出したため）
- [x] Feedback.md 更新（forked lint エージェントのコンテキスト欠如パターンを記録）
- [x] TODO.addf.md・Plan 0040 の状態更新（フェーズ1・2完了・完了条件チェック・
      「要オーナー確認」1点をエージェント判断で解消）
- [ ] コミット

#### 日記

##### 2026-07-14 — ccchain フェーズ2（オプトイン配布機構）組み上げ
**やったこと**: オーナーからフェーズ2着手の直接指示。Plan 0040 本文どおり GUI テストの
3原則（原本が真実源・有効化コピーは使い捨て・乖離時は WARNING）を踏襲しつつ、ccchain 固有の
差分（外部バイナリ依存・設定ファイルが `.claude/commands/` ではなくプロジェクトルート・
settings.json への JSON レベルのフック追記）を反映した専用スクリプト `sync-ccchain.py` を
新設した。既存の `sync-optional-skills.py` をそのまま流用せず別スクリプトにしたのは、
「.ccchain.conf は初回配置後に書き換え可能」という要件が GUI テストの「原本と有効化コピーは
常に同一であるべき」という前提と根本的に矛盾するため。サンドボックスで8シナリオを手動検証してから
自動テスト化し、テストハーネス自身のバグ（`run_sync` 関数内の `cd -` が `$?` を上書きし、
WARNING が出ているのに exit 0 と誤判定していた）も発見・修正した。
**今の見立て**: フェーズ2の機構自体は健全に動作する（サンドボックステスト23件全パス）。
ただし ADDF 自身は今もフェーズ1の手組み配線（settings.local.json）を使っており、
フェーズ2機構（settings.json 限定）とは意図的に未統合のまま。`/addf-lint` の forked
エージェントがこれを「Q5と矛盾する状態」と誤解して報告してきたため、knowhow に
「意図的な分離であり bug ではない」ことを明記した。
**次の自分へ**: フェーズ3（ガイド `.claude/addf/guides/ccchain-setup.md` 新設・
`/addf-migrate` 案内統合）とフェーズ4（フェーズ1手組み配線の撤去・
`addf-Behavior.toml` の `[ccchain] enable = true` への切替）は Plan 0040 本文どおり
「まず数タスク分運用してから」の設計を尊重し、まだ着手しないこと。次に着手するときは
`.claude/addf/knowhow/ADDF/ccchain-dogfooding-phase1.md` の「フェーズ1とフェーズ2の
配線先は意図的に別ファイル」の節を先に読むこと。
**気になっていること**: `sync-ccchain.py` の `.gitignore` エントリ判定・
`check_gitignore_consistency` 相当の検査（sync-optional-skills.py にはあるが
sync-ccchain.py には実装していない）。今回は `.gitignore` を手動で移動したため実害は
無いが、将来 `.ccchain.conf`/`.ccchain.local.conf` の gitignore 列挙漏れを機械検出する
仕組みが無い状態。フェーズ3以降で必要性を再検討してよい。
