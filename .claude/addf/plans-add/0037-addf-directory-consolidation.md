...
## 実装状況: 一部完了（フェーズ1 完了 2026-07-06。フェーズ2=本体移行・フェーズ3=migrate 統合とリリースが残り）
...
> 本節の実施様式の一般定義は `docs/guides/speculative-development.md` の
...
- ADDF 管理サブディレクトリ（docs/plans 等）**内部**の非所有ファイルは無条件に巻き込まれる仮定が残る
  （合成テストは docs/knowhow の独自記事のみ検証。knowhow はダウンストリーム独自記事も「仕組みの一部として
