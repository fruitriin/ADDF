...
- **案B（スキル本文への埋め込み一体化方式）**
  - 理由: スキル本文への埋め込みは PR 面積を大きく膨張させ、分離維持のコスト（現行方式の同期不良観測）を上回る便益がない
  - 証拠: 2026-07-10 実測（ADDF 本体6ファイル＋ダウンストリーム2ファイルを対象に、実運用中の .exp.md 内容量とスキル本文の比較。詳細は本 Plan 項目1の実測記録）
  - 復活条件: 現行分離方式のもとで、スキル本文と .exp.md の同期不良が実運用で頻発した場合
