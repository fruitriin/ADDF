...
## 実装状況: 一部完了（項目4 完了・項目1〜3 は Questions Q3 応答待ち）

- **項目4 完了（2026-07-07）**: @メンション解決のパストラバーサル耐性を Python (`lint-template-sync.py` の `_safe_resolve_mention`) / Bash (`verify-checksums.sh` の `detect_repo_kind`) の両側に追加。`..` を含むパス・絶対パス・シンボリックリンク経由の脱出を silent に無視。test-template-sync.sh に Test 20/20b/20c を追加（3種の脱出手法が downstream 判定を成功させないことを機械保証）。ペア7 の同期契約で両側の整合を保つ
- **項目1〜3 未着手**: 「不便のない範囲で」の指針とオーナー確認事項があるため Questions Q3 に投下。応答後に着手する
  - 項目1（deny ルール）: 対象リスト確認待ち
  - 項目2（addf-init 実物レビュー）: preview 対象カテゴリ確認待ち
  - 項目3（破壊的 git 対策）: 検出範囲・モード連動確認待ち
