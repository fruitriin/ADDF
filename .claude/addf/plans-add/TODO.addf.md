...
| **1** | 58 | `.claude/addf/plans-add/0058-dashboard-html-review-ui.md`（検討スタブ） | 未着手・**優先着手指示あり**（2026-07-16 起票。オーナー発案。「レビューのボトルネック感がヤバい」ため優先着手のオーナー指示。設計方向は二層構造に確定: ミクロ層〔1機能分の確認〕= crit.md をそのまま採用・マクロ層〔俯瞰〕= VitePress ローカル専用インスタンスで「ダッシュボード＋プランビューア」。ページ構成4ページ確定〔要フィードバック／進行中タスク／Questions／未実施計画〕。FB 判定は状態管理エリアへの「オーナーフィードバック有無」明示フィールド方式。叩き〔HTML モック〕提示済み — 合意が取れたら標準テンプレ昇格・着手） |
