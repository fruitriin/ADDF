...
| **1** | 54 | `.claude/addf/plans-add/0054-settings-self-write-protection.md`（検討スタブ） | 未着手（2026-07-11 起票。Plan 0053 の doc-review で Plan 0026 の [Critical]「settings.json/hooks 自己書き換え保護」が Plan 0043 で明示的にスコープ外とされたまま独立 Plan 未作成で放置されていたことが判明。Progress 運用ルール7〔主題外Critical最優先切り出し〕に従い起票。D 軸実装完了 2026-07-17 — settings.json ask 8ルール〔settings・local・hooks・.ccchain.conf のルートアンカー Edit/Write〕＋ccchain args の cp/mv 宛先保護。code-review High/Medium 反映済み。残: ask 発火の実地確認〔オーナー回答待ち〕・init/migrate 二重確認の事後観測） |
| 3 | 56 | `.claude/addf/plans-add/0056-plan-genealogy-tree.md` | 一部完了（フェーズ1・2 完了 2026-07-17 — edge 基盤・系統樹ページ〔Mermaid・クリティカルパス区分〕・lint-genealogy・31 Plan 遡及・code-review Critical 2件含む6件反映。残: オーナーのブラウザ目視確認のみ〔owner_feedback: 待ち〕） |
...
| 3 | 68 | `.claude/addf/plans-add/0068-residual-path-scheme-detection.md` | 未着手（2026-07-17 起票。Plan 0059/0060 レビュー残件 — blob URL 自己参照の検出漏れ・basename 衝突誤検知の根治〔URL スキーム検出設計〕・compile_pattern 同期契約のペア9 lint 化・CI downstream 模擬。2体独立指摘のコンセンサス High） |
...
リリース後タスク:
- `/addf-overview` full 再生成（v0.7.0 リリース時の patch 申し送り — オプトイン2カテゴリ化・ダッシュボード要素増・phase-flows/interactions 未反映。詳細は addf-overview.exp.md）
