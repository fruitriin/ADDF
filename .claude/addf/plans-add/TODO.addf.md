...
| 3 | 58 | `.claude/addf/plans-add/0058-dashboard-html-review-ui.md`（検討スタブ） | 未着手（2026-07-16 起票。オーナー発案。設計方向はオーナー回答で二層構造に確定: ミクロ層〔1機能分の確認〕= crit.md〔ローカルファーストレビューツール・MIT・Claude Code プラグインあり〕をそのまま採用・マクロ層〔俯瞰〕= VitePress 自作で「ダッシュボード〔詰まり・進行中タスク〕＋プランビューア〔Plan 0056 系統樹の載せ先候補〕」の2本柱。残る問いはマクロ層のページ構成・インスタンス配置〔公開サイト同居 or ローカル専用〕等） |
