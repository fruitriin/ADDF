...
`docs/guides/speculative-development.md`「one-shot（一発通し切り）」）の事前清算条件が
...
2. **one-shot マーカー付き未着手 Plan の存在確認**:
   ...
   grep -rl '^execution_style: one-shot' --include='*.md' docs/plans docs/plans-add 2>/dev/null
   # TODO の状態注記（未着手 × one-shot）からも拾う（マーカー導入前の既存 Plan 対応）
   grep -hE '未着手.*one-shot|one-shot.*未着手' TODO.md docs/plans-add/TODO.addf.md 2>/dev/null
   ```
...
3. 1・2 の両方が成り立てば**窓が開いている**。次の投機を始める前にオーナーへ選択を提示する:
   ...
   - `relaxed` / `unattended`: `.claude/Questions.md` に同じ質問を置き、**応答待ちの間はこの
...
- **判定理由を1行残す**: 投機する概念は `.claude/Worktrees.md` の表の下に
...
1. Plan 草案を `docs/plans/`（ADDF 本体では `docs/plans-add/`）に起こし、TODO に登録する
   （書式: `.claude/templates/PlanTemplate.md`。詰め切れなければ検討スタブ variant でよい）
2. 大改造級（ディレクトリ構成の全面再編など全域に触れるもの）は、Plan の実装状況ヘッダ直後に行頭で
   `execution_style: one-shot` マーカーを付けて起案する（一発通し切り — 意味と実施様式の
   単一ソースは `docs/guides/speculative-development.md`「one-shot」、書き方は PlanTemplate 参照）。
...
`docs/guides/speculative-development.md` を参照）。
...
5. **[A-5]** `.claude/Worktrees.md` の該当行を状態「昇格済み」に更新する
...
1. **[B-1]** 手順9 で push 済みの `speculative/<concept>` から PR を作成する:
   ...
   PR 本文は `docs/guides/pr-format.md` の規約に従い、投機 PR では加えて以下を記載する:
...
2. **[B-2]** `.claude/Worktrees.md` の該当行に PR 番号を注記する（状態は変えない — PR 作成は
...
2. **[後-2]** `.claude/Worktrees.md` の該当行を状態「昇格済み」に更新する
...
- Pending 在庫は **5本まで**を許容する。6本以上になったら Dashboard（unattended 時）/
  `.claude/Questions.md` でオーナーに整理（再開 or 放棄）を提案する。件数は reconcile check
