...
| 9. `.claude/addf/addfTools/migrate-paths.py` ⇔ `.claude/addf/addfTools/lint-residual-paths.py` | `compile_pattern` 同期ブロック（`# --- BEGIN/END sync: compile_pattern (Plan 0068) ---`）の正規化テキスト一致。Plan 0068 で導入した URL スキーム検出設計の同期契約 | WARNING |
