...
| 7. `.claude/addfTools/verify-checksums.sh` ⇔ `.claude/addfTools/lint-template-sync.py` | `detect_repo_kind()` Python⇔Bash 実装の同期契約文言の存在チェック（挙動比較は困難なため契約明示を機械保証） | WARNING |
...
`docs/plans-add/`・`docs/plans/` の Plan ファイルについて、`## 実装状況:` ヘッダが
「完了」で始まるのに完了条件セクションに未チェック `- [ ]` が残っている矛盾
（フェーズ分割 Plan の途中マージで「済み」に見える誤完了）を検出する:

```bash
uv run --python 3.11 .claude/addfTools/lint-plan-status.py
```
