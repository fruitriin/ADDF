...
```bash
uv run --python 3.11 .claude/addf/tools/lint-json.py
```
...
```bash
uv run --python 3.11 .claude/addf/tools/lint-hooks-exec.py
```
...
```bash
uv run --python 3.11 .claude/addf/tools/lint-frontmatter.py
```

## 4. addf-Behavior.toml 構文チェック

```bash
uv run --python 3.11 .claude/addf/tools/lint-toml.py
```
...
| 7. `.claude/addf/tools/verify-checksums.sh` ⇔ `.claude/addf/tools/lint-template-sync.py` | `detect_repo_kind()` Python⇔Bash 実装の同期契約文言の存在チェック（挙動比較は困難なため契約明示を機械保証） | WARNING |

※ lint にペアを追加・変更したら、この表とスクリプト docstring も同時に更新する。

```bash
uv run --python 3.11 .claude/addf/tools/lint-template-sync.py
```
...
```bash
uv run --python 3.11 .claude/addf/tools/lint-checklist.py
```
...
```bash
uv run --python 3.11 .claude/addf/tools/sync-optional-skills.py
```
...
```bash
uv run --python 3.11 .claude/addf/tools/lint-plan-status.py
```
