...
## 実装状況: 一部完了（実装・ローカル検証済み 2026-07-06。CI（Linux）での照合テスト通過確認のみ残）
...
2. **Swift ビルドの決定性** → **同一環境内のみ決定的**（2026-07-06 実測: swiftc 6.3.2 / arm64-macosx26.0）
   ...
   - よって第二段階の機械検証（ソース⇔バイナリの再現検証）は toolchain 固定なしでは成立しない。
     **運用**: ソース変更時は `bash .claude/addfTools/build.sh` で再ビルド →
...
- [x] `bash .claude/tests/run-all.sh` にバイナリ照合テストが含まれ通過する（macOS ローカルで
      15/15 PASS を確認。ハッシュ計算のみでバイナリ実行不要のため非 macOS でも SKIP しない設計）
- [ ] CI（ubuntu-latest）で test-binary-checksums.sh が通過する（Plan 0030 の CI が run-all.sh 経由で
      自動実行する — 次回 push 後の CI 実行で確認）
- [x] `build.sh` 実行で checksums が更新され、バイナリだけ・checksums だけの片側コミットを
      テストが検出できる（ドリフト注入テスト Test 3/4 で FAIL を実測）
- [x] addf-init 配布物で照合が成立する（addf-init / addf-migrate は `.claude/addfTools/` を
      ディレクトリ丸ごとコピーするため checksums.sha256 とテストは自動的に配布物に含まれる。
      加えて旧配布＝checksums 不在時のダウンストリーム挙動を明示 SKIP として定義済み）

## 実装記録（2026-07-06）

- `.claude/addfTools/build.sh` — ビルド完了時に `generate_checksums` を実行。
  `--checksums-only` フラグでビルドなし再生成（swiftc 不要・全 OS 可。テストの生成経路検証にも使用）。
  sha256sum → shasum -a 256 フォールバック
- `.claude/addfTools/checksums.sha256` — 新規・コミット対象。現行コミット済みバイナリ4種から生成
- `.claude/addfTools/verify-checksums.sh` — 照合スクリプト（テスト・CI・手動から共用）。
  exit 0=OK/正当 SKIP、1=ERROR、2=WARNING（addfTools 3値規約）
- `.claude/tests/tools/test-binary-checksums.sh` — 照合テスト8ケース（15 アサーション）。
...
- **W5: FAIL に復旧コマンド案内** — バイナリ不在・ハッシュ不一致の FAIL メッセージに、
  `bash .claude/addfTools/build.sh` 呼び出しと確認手順を併記
- **W6: build.sh の checksums 上書き案内** — `docs/guides/gui-test-setup.md` に「実行すると
