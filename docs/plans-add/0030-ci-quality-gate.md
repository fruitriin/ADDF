...
## 実装状況: 一部完了（実装済み。初回 CI 実行の確認待ち）

> 実装は完了（`.github/workflows/test.yml`・`.github/scripts/run-lint.sh`・README バッジ・setup.md 注記）。
> CI はローカルで完全検証できないため、push 後の初回 Actions 実行で
> 完了条件の未チェック2件を確認してから完了化する。
...
- `docs/guides/setup.md`（「実運用の参考」に1行追記）
...
5. **ダウンストリーム配布形態** — **配布しない**（addf-init のコピーリストに `.github/` が
   含まれないことを確認済み — 変更不要）。ガイド例示は `docs/guides/setup.md` の
...
- [ ] 初回 CI 実行の確認:
  (1) 緑の実行を確認する
  (2) **意図的に ERROR を注入した使い捨てブランチを push し、fail することを確認する**（検証後ブランチを削除）
  (3) ダミー WARNING を注入して `::warning::` annotation の表示を確認する（(2) と同様の使い捨てブランチで）
  (4) ubuntu ランナーで macOS 専用テストが SKIP としてログの Results 行に計上されることを確認する — silent truncation にしない
- [ ] オーナーが branch protection（`test` を required check に設定するか）の要否を判断する <!-- human-judgment -->
