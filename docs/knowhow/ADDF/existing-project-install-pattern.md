...
| カテゴリ | 処理 | 例 |
|---|---|---|
| 無条件コピー | `addf-` プレフィックス／専用ディレクトリで衝突リスクなし。ただし `*.addf.md` は**コピーしない**（ADDF 本体専用サフィックス — 存在ベース判定を汚染するため） | commands/addf-*.md, agents/addf-*.md, hooks/, docs/knowhow/ADDF/, docs/guides/, addfTools/, tests/, templates/, optional/ |
...
- 無条件コピーカテゴリに「`*.addf.md` を除外する」原則を追記
  ...
  存在≠所有 の分離規約に対応）
- 干渉チェック3カテゴリの例示リストを現状追従（docs/knowhow/ADDF/, docs/guides/, optional/, tests/ 等を追加）
