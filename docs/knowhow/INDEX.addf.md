...
| [ADDF/rule-placement-execution-guarantee.md](ADDF/rule-placement-execution-guarantee.md) | エージェント運用ルールの配置先と実行保証。参照では実行されない、実行主体が必ず読むファイルにコア手順をインライン展開する | 実行保証, インライン展開, サブエージェント定義, 運用ドキュメント, ProgressTemplate, 集約ルール, 参照, メインエージェント |
