...
| 🟢 2026-06-11 | [ADDF/viteplus-bun-frontend-bootstrap.md](ADDF/viteplus-bun-frontend-bootstrap.md) | Vite+ (vp) + Bun でのフロントエンド基盤立ち上げ。bunx の bin 解決罠・vp create の Vue 化・バージョンピン採番・rust-embed + build.rs での SPA 同梱・Vitest/Playwright テスト基盤の実値知見 | Vite+, vp, Bun, bunx, Vue, UnoCSS, Pinia, rust-embed, build.rs, Vitest, Playwright, oxfmt, oxlint, golden, HMR |
| 🟢 2026-06-11 | [ADDF/milestone-gated-ui-plan.md](ADDF/milestone-gated-ui-plan.md) | 目視判定ゲート付き UI 計画の運用。判定ゲートは進行の同期点・URL 全状態同期で判定素材を安価に・機能等価移植は潜在バグの炙り出し装置・画素等価検証の方法論・クロス検証テスト | 判定ゲート, マイルストーン, 目視判定, UI 計画, URL 同期, スクショ, 画素等価, 機能等価移植, クロス検証, toBeHidden, 潜在バグ |
| 🟢 2026-06-11 | [ADDF/deprecated-module-maintenance-policy.md](ADDF/deprecated-module-maintenance-policy.md) | deprecated モジュールへの保守方針コメント。削除予定フェーズ明記・判定日つき投資抑制根拠・優先ルール (バグ修正は受ける/品質投資は避ける) の3点セット。捨てる前提のファイル隔離も併記 | deprecated, 保守方針, 投資抑制, 段階的移行, リメイク, 削除予定, 判定日, Temporary Isolation, ファイル隔離 |
